...
class PythonConsoleWidget(QtGui.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # out_clear_btn.setIcon(QtGui.QIcon(":/console/clear.png"))
        # out_clear_btn.setIcon(QtGui.QIcon(":/console/open.png")
        ...
        open_file_icon = QtGui.QIcon(":/console/open.png")
        ...
        self._settings = QtCore.QSettings(
            "Autodesk, Inc.", "Alias Python API Python Console"
        )

        tab_info_list = self._settings.value("tab_info", None)

        if tab_info_list is None:
            self.tabs.add_tab()
        else:
            for tab_info in tab_info_list:
                self.tabs.add_tab(
                    name=tab_info.get("tab_name"), contents=tab_info.get("tab_contents")
                )

        cur_tab_index = self._settings.value("current_tab", 0)
        self.tabs.setCurrentIndex(cur_tab_index)

        width = self._settings.value("width", 800)
        self.setMinimumWidth(width)
        # self.setMaximumWidth(width)

        height = self._settings.value("height", 600)
        self.setMinimumHeight(height)
        # self.setMaximumHeight(height)

        style_qss = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "style.qss"
        )
        with open(style_qss, "rt") as style_sheet_file:
            qss_data = style_sheet_file.read()
            self.setStyleSheet(qss_data)

    def closeEvent(self, event):
        """
        Override the Qt closeEvent method to save the state settings before closing.
        """

        self._save_settings()
        super(PythonConsoleWidget, self).closeEvent(event)
    ...
    def _save_settings(self):
        ...
        self._settings.setValue("tab_info", self.tabs.get_tab_info())
        self._settings.setValue("current_tab", self.tabs.currentIndex())
